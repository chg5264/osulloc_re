	...
	<div class="wrap">
		...
		<div class="container">
			...
			<form id="productForm" method="post">
				...
				<div>
					<input type="hidden" name="name" value="${productse.name}">
				</div>
				<div>
					<input type="hidden" name="price" value="${productse.price}">
				</div>
				<div>
					<input type="hidden" name="sale" value="${productse.sale}">
				</div>
				<div>
					<input type="hidden" name="filename" value="${productse.filename}">
				</div>
						...
						<div class="detail_b">
							...
							<div class="button_b">
								...
								<button type="button" name="buy" id="buy">
									<a href="#" class="btn"> <span>바로구매</span>
									</a>
								</button>
							</div>
						</div>
			...
			<div id="reviewD">
				...
				<c:if test="${login != null}">
					<div class="reviewW">
						<a href="/osulloc/page/reviewWrite?prodnum=${productse.prodnum}">리뷰
							작성하기</a>
					</div>
				</c:if>
				...
				<ul>
					<c:forEach items="${productcri}" var="productreview">
						...
						<c:if test="${productreview.prodnum == productse.prodnum}">
							...
							<input type="hidden" class="pageNum" name="pageNum"
								value="${pageMaker.cri.pageNum}">

							<li><input type="hidden" class="relistPno" value="0">

								<div id="r_detail">
									<input type="hidden" id="pno${productreview.pno}"
										value="${productreview.pno}">
									<div class="pno" hidden>${productreview.pno}</div>
									<div class="uploadResult">
										<img src="/osulloc/display?fileName=${productse.filename}"
											alt="">
									</div>
									<div class="detail_right">
										...
										<c:if test="${login != null}">
											...
											<div class="more" id="more${productreview.pno}"
												...
												<a
													href="/osulloc/page/reviewModify?prodnum=${productse.prodnum}&pno=${productreview.pno}">수정</a>
												<a
													href="/osulloc/page/reviewdelete?prodnum=${productse.prodnum}&pno=${productreview.pno}">삭제</a>
											</div>
										</c:if>
									</div>
								</div>

								<button class="mrebtn addReplyBtn"
									data-pno="${productreview.pno}">댓글</button>
								<div class="reply" id="reply${productreview.pno}">

									<ul class="relist" id="relist${productreview.pno}"
										data-pno="${productreview.pno}"></ul>
								</div>
								<div class="rewrite" id="rewrite${productreview.pno}"
									data-pno="${productreview.pno}"></div></li>
						</c:if>
				...
				<form id="actionForm" action="/osulloc/page/detailProduct"
					method="get">
					<input type="hidden" name="prodnum" value="${productse.prodnum}">
					...
					<div class="page">
						<ul class="pagination">
							...
							<c:forEach var="num" begin="${pageMaker.startPage}"
								...
								<li class="paginate_button page-item pageC"><a
									href="${num}" class="page-link PNum">${num}</a></li>
							</c:forEach>
