...
public class ReplyReviewController {
	...
	@PostMapping(value = "new", consumes = "application/json", produces = { MediaType.TEXT_PLAIN_VALUE }) // MediaType.TEXT_PLAIN_VALUE:문자열로
																											// 반환해라
	...
	@GetMapping(value = "list/{pno}", produces = { MediaType.APPLICATION_JSON_UTF8_VALUE }) // produces="select된 결과를 반환할
																							// 타입"
	public ResponseEntity<ArrayList<ReplyReviewDTO>> getList(@PathVariable int pno) {// ReplyDTO타입으로 된 결과를 return해라
		// @PathVariable : Rest방식에서 주로 사용. url경로의 일부를 파라미터(변수) 사용하고자 할 때 작성한다. 경로의 명과
		// 변수명이 같아야 한다.
		System.out.println(pno);
		return new ResponseEntity<>(rservice.list(pno), HttpStatus.OK);// 자바스크립트의 sucess부분으로 간다.//"success"는 String타입지만,
																		// 위에는 ReplyDTO타입이여서 에러가 난다.
	...
	@PutMapping(value = "update", consumes = "application/json", produces = { MediaType.TEXT_PLAIN_VALUE }) // 같은
																											// value값을
																											// 사용해도 된다.
																											// insert==update
